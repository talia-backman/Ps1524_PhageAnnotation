...
# max number of systems in one bacterial strain
max(tree_df2$system_type_count) # 12

# subset to only most common anti-phage systems (sub-type)
tree_df2 <- subset(tree_df2, subtype == "Abi2" | subtype == "RM_Type_IV" | 
                            subtype == "RM_Type_I" | subtype == "RM_type_II")
